...
describe('GameProgress エッジケース', () => {
  describe('境界値テスト', () => {
    ...
    it('最終シーン（99）で次に進むとルートがクリアされる', () => {
      ...
      expect(progress.getCurrentScene().getValue()).toBe(
      ...
      expect(progress.getCurrentScene().getValue()).toBe(
        TEST_CONSTANTS.FINAL_SCENE_NUMBER
      );
  ...
  describe('保存時間の管理', () => {
    ...
    it('ルート選択時に保存時間が更新される', async () => {
      ...
      expect(progress.getSaveTime().getTime()).toBeGreaterThan(
        initialSaveTime.getTime()
      );
    });
  });
});
