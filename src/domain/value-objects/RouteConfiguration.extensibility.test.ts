...
describe('RouteConfiguration 拡張性テスト', () => {
  ...
  describe('トゥルールート解放条件の拡張性', () => {
    it('DLCルートがあってもトゥルールート解放条件はベースルートのみ', () => {
      ...
      expect(RouteConfiguration.isTrueRouteUnlockCondition(baseRoutesCleared)).toBe(true);
      
      // DLCルートも含めてクリア
      const allRoutesCleared = ['route1', 'route2', 'route3', 'dlc-route-1', 'dlc-route-2'];
      expect(RouteConfiguration.isTrueRouteUnlockCondition(allRoutesCleared)).toBe(true);
      
      // DLCルートのみクリア
      const dlcOnlyCleared = ['dlc-route-1', 'dlc-route-2'];
      expect(RouteConfiguration.isTrueRouteUnlockCondition(dlcOnlyCleared)).toBe(false);
    });
    ...
    it('ベースルートが一部未クリアだとDLCルートクリアでも解放されない', () => {
      ...
      expect(RouteConfiguration.isTrueRouteUnlockCondition(partialCleared)).toBe(false);
    });
  });
  ...
  describe('大規模拡張の性能テスト', () => {
    ...
    it('大量のルートがあってもisValidRouteが高速', () => {
      ...
      // 複数のルートの妥当性をチェック
      const testRoutes = ['route1', 'dlc-route-500', 'non-existent-route', 'trueRoute'];
  ...
  describe('設定リセット機能', () => {
    it('resetConfiguration後は初期状態に戻る', () => {
      ...
      expect(RouteConfiguration.getBaseRoutes()).toEqual(['route1', 'route2', 'route3']);
    });
  });
});