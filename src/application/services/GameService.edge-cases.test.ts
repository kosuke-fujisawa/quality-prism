...
describe('GameService エッジケース', () => {
  ...
  beforeEach(() => {
    mockGameProgressRepository =
      mockRepositories.createGameProgressRepository();
    mockGameSettingsRepository =
      mockRepositories.createGameSettingsRepository();
  ...
  describe('複合エラーシナリオ', () => {
    it('リポジトリ保存失敗時の状態一貫性', async () => {
      const mockProgress = GameProgress.createNew(
        TEST_CONSTANTS.DEFAULT_TEST_ID
      );
      mockGameProgressRepository.getOrCreate = vi
        .fn()
        .mockResolvedValue(mockProgress);
      mockGameProgressRepository.save = vi
        .fn()
    ...
    it('getOrCreate失敗後のadvanceToNextScene', async () => {
      ...
      await expect(gameService.advanceToNextScene()).rejects.toThrow(
        'データベース接続エラー'
      );
    });

    it('設定取得失敗時のperformAutoSave', async () => {
      mockGameSettingsRepository.get = vi
        .fn()
        .mockRejectedValue(new Error('設定読み込みエラー'));

      await expect(gameService.performAutoSave()).rejects.toThrow(
        '設定読み込みエラー'
      );
    ...
    it('設定更新中のエラー処理', async () => {
      ...
      mockGameSettingsRepository.save = vi
        .fn()
        .mockRejectedValue(new Error('設定保存エラー'));

      await expect(gameService.updateGameSettings(0.8)).rejects.toThrow(
        '設定保存エラー'
      );
  ...
  describe('タイムアウト処理', () => {
    it('長時間実行処理のタイムアウト', async () => {
      ...
      const slowPromise = new Promise((resolve) => {
        setTimeout(
          () => resolve(GameProgress.createNew(TEST_CONSTANTS.DEFAULT_TEST_ID)),
          5000
        );
      });
  ...
  describe('データ整合性', () => {
    it('複数の設定変更の原子性', async () => {
      ...
      // 複数の設定を同時に変更
      await expect(
        gameService.updateGameSettings(0.8, 2.0, false)
  ...
  describe('エラーメッセージの一貫性', () => {
    it('様々なエラーシナリオでの適切なメッセージ', async () => {
      ...
      mockGameProgressRepository.getOrCreate = vi
        .fn()
        .mockRejectedValue(new Error(ERROR_MESSAGES.DATABASE_ERROR));

      const result1 = await gameService.selectRoute(TEST_CONSTANTS.ROUTE1);
      enhancedAssertions.expectGameServiceResult(
        result1,
        false,
      ...
      mockGameProgressRepository.getOrCreate = vi
        .fn()
        .mockResolvedValue(
          GameProgress.createNew(TEST_CONSTANTS.DEFAULT_TEST_ID)
        );

      const result2 = await gameService.selectRoute(
        TEST_CONSTANTS.INVALID_ROUTE
      );
      enhancedAssertions.expectGameServiceResult(
        result2,
        false,
